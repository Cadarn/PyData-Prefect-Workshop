...
tweet_text = data.query("airline_sentiment == @random_tweet")[["text"]].sample(1).iat[0,0]
st.sidebar.markdown(tweet_text)

st.sidebar.markdown("### Num of tweets by sentiment")
select = st.sidebar.selectbox("Visualisation type", ["Histogram", 
                                                     "Pie chart"], 
                                                     key="1")
...
choice = st.sidebar.multiselect('Pick airlines', tuple(data.airline.unique()), key="0")
